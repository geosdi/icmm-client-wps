 ...
 */
@DescribeProcess(title = "World State Publication in Seq",
        description = "CRISMA WPS for publishing world states in sequence")
public class WorldStatePublicationSeq implements GeoServerProcess {
    ...
    public WorldStatePublicationSeq(GeoServerUtils geoServerUtils, ICMMHelperFacade icmmHelperFacade) {
    ...
    public String execute(
            @DescribeParameter(name = "wsStartID", description = "ID of the starting world state") int wsStartID,
            @DescribeParameter(name = "wsEndID", description = "ID of the latest WS to publish") int wsEndID)
            throws Exception {

        if (wsStartID < wsEndID) {
            throw new IllegalArgumentException("The passed parameters are not valid, "
                    + "the ws end id must be minor than the ws start id");
        }
        int noOfEvents = wsEndID - wsStartID + 1;
...
//                SimpleFeature eqTDVPar = iterator.next();
                ...
                int targetDBWorldStateID = 0;
                if (targetDBWorldStateID == 0) {
                    targetDBWorldStateID = wsStartID;
                } else {
                    targetDBWorldStateID = targetDBWorldStateID + 1;
                }
